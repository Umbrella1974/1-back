...
def load_font_safe(size, is_chinese=False):
    ...
    for font_name, font_desc in windows_fonts:
        try:
            font = pygame.font.SysFont(font_name, size)
            test_surface = font.render("中文测试", True, (0, 0, 0))
            if test_surface.get_width() > 20:
                print(f"✓ 使用系统字体: {font_desc} ({font_name})")
                return font
        except:
            continue
    ...
    try:
        ...
        # 优先尝试包含中文关键词的字体
        chinese_keywords = ['song', 'hei', 'kai', 'fang', 'ming', 'yuan', 'cjk', 'noto', 'wqy']
        for font_name in all_fonts:
            if any(keyword in font_name.lower() for keyword in chinese_keywords):
                try:
                    font = pygame.font.SysFont(font_name, size)
                    test_surface = font.render("中", True, (0, 0, 0))
...
# 加载字体
print("\n加载字体...")
font_stimulus = load_font_safe(FONT_SIZE_STIMULUS, is_chinese=False)  # 数字不需要中文
font_instruction = load_font_safe(FONT_SIZE_INSTRUCTION, is_chinese=True)  # 指导语需要中文
font_feedback = load_font_safe(FONT_SIZE_FEEDBACK, is_chinese=False)  # 反馈符号不需要中文

# 测试字体渲染
print("\n字体渲染测试:")
test_texts = ["1-Back 任务", "中文测试", "按F键:相同"]
for text in test_texts:
    try:
        surface = font_instruction.render(text, True, (0, 0, 0))
        print(f"  '{text}': {surface.get_width()}x{surface.get_height()}")
    except Exception as e:
        print(f"  '{text}': 渲染失败 - {e}")
...
def show_text(text, color=TEXT_COLOR, font=None, wait_key=None, data_list=None):
    """显示文本，可选等待按键
    ...
    Args:
        ...
        color: 文本颜色
        font: 字体
        wait_key: 等待的按键（True=任意键，pygame.K_xxx=特定键）
        data_list: 数据列表，退出时需要保存
    """
    if font is None:
    ...
    for line in lines:
        try:
            ...
            # 如果渲染失败，使用默认字体
            print(f"文本渲染失败: '{line}' - {e}")
            fallback_font = pygame.font.Font(None, FONT_SIZE_INSTRUCTION)
            text_surface = fallback_font.render(line, True, color)
            text_rect = text_surface.get_rect(center=(SCREEN_WIDTH // 2, y_offset))
            screen.blit(text_surface, text_rect)
...
def show_break(completed, total, data_list=None):
    """显示休息界面"""
    text = f"已完成 {completed}/{total} 个试次\n\n请休息片刻\n\n按空格键继续"
    show_text(text, wait_key=pygame.K_SPACE, data_list=data_list)


def show_results(stats, data_list=None):
    """显示结果统计"""
    text = (
        "实验结束！\n\n"
    ...
    show_text(text, wait_key=True, data_list=data_list)


def check_key_event(event, trial_idx, stim_onset):
    """
    检查按键事件是否有效
    ...
    # 调试信息
    debug_info = f"按键检测: 键名='{key_name}', 键码={key_code}, unicode='{event.unicode}'"
    log_message(f"试次 {trial_idx + 1}: {debug_info}")

    # 检查是否是目标键
    # 方法1: 检查键名
    if key_name == KEY_SAME.lower():
        rt = pygame.time.get_ticks() - stim_onset
        return True, True, KEY_SAME, rt
    elif key_name == KEY_DIFFERENT.lower():
        rt = pygame.time.get_ticks() - stim_onset
        return True, False, KEY_DIFFERENT, rt
    ...
    if KEY_SAME.lower() in key_map and key_code == key_map[KEY_SAME.lower()]:
        rt = pygame.time.get_ticks() - stim_onset
        return True, True, KEY_SAME, rt
    elif KEY_DIFFERENT.lower() in key_map and key_code == key_map[KEY_DIFFERENT.lower()]:
        rt = pygame.time.get_ticks() - stim_onset
        return True, False, KEY_DIFFERENT, rt
    ...
    if key_code == pygame.K_f:
        rt = pygame.time.get_ticks() - stim_onset
        return True, True, 'f', rt
    elif key_code == pygame.K_j:
        rt = pygame.time.get_ticks() - stim_onset
        return True, False, 'j', rt
...
def wait_for_focus():
    ...
    while waiting:
        ...
        # 如果5秒后还没有焦点事件，假设已有焦点
        if not focus_gained and pygame.time.get_ticks() > 5000:
            log_message("⚠ 超时，假设窗口已有焦点")
            focus_gained = True
...
def main():
    """主实验流程"""
    log_message("=" * 60)
    log_message("1-Back 实验开始")
    log_message(f"配置: {NUM_TRIALS}试次, 目标比例{TARGET_RATIO}")
    log_message(f"按键: {KEY_SAME.upper()}=相同, {KEY_DIFFERENT.upper()}=不同")
    log_message("=" * 60)
    ...
    show_text(instructions, wait_key=pygame.K_SPACE, data_list=data_list)

    # 正式实验
    for trial_idx in range(NUM_TRIALS):
        # 检查是否需要休息
        if trial_idx > 0 and trial_idx % BREAK_INTERVAL == 0:
            show_break(trial_idx, NUM_TRIALS, data_list)
        ...
        while pygame.time.get_ticks() < response_window_end:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN and not response_made:
                    ...
                    # 使用新的按键检测逻辑
                    is_valid, resp, resp_key, reaction_time = check_key_event(event, trial_idx, stim_onset)
        ...
        if trial_idx == 0:
            ...
            if response is None:
                ...
                log_message(f"试次 {trial_idx + 1}: 无反应，标记为错误")
            else:
                correct = (response == is_target)
                log_message(f"试次 {trial_idx + 1}: 反应={response}, 目标={is_target}, 正确={correct}")
