...
def show_stimulus(number, duration):
    """显示数字刺激指定时长，返回开始时间"""
    draw_stimulus(number)
    start_time = pygame.time.get_ticks()

    # 在刺激持续期间保持显示
    while pygame.time.get_ticks() - start_time < duration:
        pygame.event.pump()  # 处理事件，防止窗口卡死
        clock.tick(60)
        ...
        while pygame.time.get_ticks() < response_window_end:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN and not response_made:
                    # 调试：记录所有按键
                    log_message(f"试次 {trial_idx + 1}: 检测到按键 key={event.key}, unicode={event.unicode}")
                    ...
                    if event.key == key_map.get(KEY_SAME):
                        response = True  # 认为相同
                        response_key = KEY_SAME
                        rt = current_time - stim_onset
                        response_made = True
                        log_message(f"试次 {trial_idx + 1}: 检测到按键 {KEY_SAME}, RT={rt}")

                    elif event.key == key_map.get(KEY_DIFFERENT):
                        response = False  # 认为不同
                        response_key = KEY_DIFFERENT
                        rt = current_time - stim_onset
                        response_made = True
                        log_message(f"试次 {trial_idx + 1}: 检测到按键 {KEY_DIFFERENT}, RT={rt}")
